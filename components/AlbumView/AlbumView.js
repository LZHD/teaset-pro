...
export default class AlbumView extends Component {
  ...
  changeIndex(newIndex) {
    ...
    let sheet = this['sheet' + index];
    let nextSheet = this['sheet' + newIndex];
  ...
  checkScroll(translateX) {
    ...
    let { x, width } = this['sheet' + index].contentLayout;
    let ltx = translateX,
      rtx = translateX;
    ...
    if (
      ...
      index > 0 && this['sheet' + (index - 1)].scrollX(0, true);
      index < images.length - 1 && this['sheet' + (index + 1)].scrollX(0, true);
      return true;
    }
  ...
  onTransforming(translateX, translateY, scale) {
    ...
    index > 0 && this['sheet' + (index - 1)].scrollX(ltx, false);
    index < images.length - 1 &&
      this['sheet' + (index + 1)].scrollX(rtx, false);
  }

  onWillInertialMove(translateX, translateY, newX) {
    return this.checkScroll(newX);
  }

  onWillMagnetic(translateX, translateY, scale) {
  ...
  renderImage(index) {
    ...
    return (
      <AlbumSheet
        ...
        ref={sheet => (this['sheet' + index] = sheet)}
        key={'sheet' + index}
      />
    );
  }
  ...
  render() {
    ...
    return (
      <View
        ...
        {...others}>
        {images.map((item, i) => this.renderImage(i))}
        {control}
      </View>
    );
  }
